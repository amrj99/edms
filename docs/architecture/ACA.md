...
## الإصدار 2.1 — Approved Architecture Baseline

**التاريخ:** 2026-07-01 | **الحالة:** معتمد رسمياً — مجمَّد للمرجعية
...
الكيان القانوني — شركة، مكتب استشاري، جهة حكومية، فرد. تملك بياناتها، تضم مستخدميها، تتحمل مسؤوليتها القانونية. مستقلة تماماً عن غيرها.
...
سياق عمل محدد زمنياً وجغرافياً. تملكه مؤسسة واحدة. يضم أطرافاً من مؤسسات متعددة. ينتهي بأرشفة رسمية.
...
**المفهوم المفقود في البنية الحالية — جوهر الامتداد المستقبلي.**

Party = مؤسسة تؤدي دوراً محدداً في مشروع محدد.

```
AECOM (Organization) ثابتة
AECOM كـ "Designer" في مشروع X = Party A
AECOM كـ "Owner Rep" في مشروع Y = Party B
نفس المؤسسة، دورين مختلفان، سياقان مختلفان، صلاحيتان مختلفتان
```
...
|---|---|---|---|
| **Party Invitation** | دعوة مؤسسة للانضمام كطرف | رسمية تعاقدية | قابل للسحب |
...
```
مشروع حالي (internal):
  collaborationMode = INTERNAL (default)
...
مشروع جديد مع تعاون (opt-in):
  ...
  → orgScopedWhere يبقى سليماً للـ internal queries
```
...
1. `project_parties` table (org + project + role + dateRange)
2. `collaborationMode` على المشروع (internal / collaborative)
3. **Authorization Path منفصل** — لا OR على `orgScopedWhere`:
   - استعلام مستقل يتحقق من: هل المستخدم party member؟ هل المشروع collaborative؟ هل الـ Resource ضمن الـ Ceiling الافتراضي للدور؟
...
- كل مشاريع internal — لا أثر
- منطق permissions الداخلي — لا تغيير

**معايير نجاح الـ Pilot:**
- Org B ترى مستندات Org A في المشروع المحدد فقط
- Org B لا ترى أي شيء خارج هذا المشروع
- Org A الـ internal projects: orgScopedWhere يعمل بلا تأثير
- Audit Trail يُسجِّل كل وصول cross-org بوضوح
