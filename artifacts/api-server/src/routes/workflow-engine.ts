...
// DEBT-010: these run INSIDE the caller's withTenant() tx (db proxy → active tx).
// The previous fire-and-forget try/catch swallow is removed: a failure now rolls
// back atomically with the workflow mutation it accompanies (a swallowed error
// inside a tx would abort the tx and break the next statement anyway).
...
// DEBT-010: DB side (task lifecycle + in-app notifications) runs INSIDE the
// caller's withTenant() tx and RETURNS the data needed to send the stage email.
// The caller dispatches that email AFTER commit (external I/O outside the tx).
// Returns null when there are no recipients (nothing to email).
type StageEmailBundle = {
  recipients: { userId: number; email: string; name: string }[];
  email: {
    stageName: string;
    stageRole?: string;
...
async function prepareStageNotification(
  ...
  // In-app notifications (RLS table — inside the tx)
...
/** Send the stage email AFTER the tx commits. Non-fatal (best-effort). */
async function dispatchStageEmail(bundle: StageEmailBundle | null): Promise<void> {
  if (!bundle) return;
  try {
    await dispatchNotification({
      event: "workflow_stage_reached",
      recipients: bundle.recipients,
      sendEmail: (to) => sendWorkflowStageEmail({ to, ...bundle.email }),
    });
  } catch { /* non-fatal */ }
}
  ...
  try {
    ...
    await withTenant(async () => {
      ...
      if (documentTypeId != null) {
        ...
        if (!docType) { result = { status: 400, body: { error: "documentTypeId does not exist for this organization" } }; return; }
      ...
      result = { status: 201, body: { ...tpl, stages: [] } };
    });
  ...
  try {
    ...
    await withTenant(async () => {
      ...
      const [tpl] = await db.update(wfTemplatesTable)
        ...
        .returning();
      result = tpl ? { status: 200, body: tpl } : { status: 404, body: { error: "Not found" } };
    });
    res.status(result!.status).json(result!.body);
  } catch (err: any) {
    logger.error({
...
router.post("/templates/:id/stages", requireRole("admin", "project_manager", "system_owner"), async (req, res): Promise<void> => {
  ...
  try {
    ...
    await withTenant(async () => {
      ...
      if (!tpl) { result = { status: 404, body: { error: "Template not found" } }; return; }

      const { name, description, responsibleRole, responsibleUserId, isTerminal, stageOrder, slaDays, reminderDays } = req.body;
      if (!name) { result = { status: 400, body: { error: "name is required" } }; return; }
      ...
      if (responsibleRole != null && !isValidAppRole(responsibleRole)) {
        result = { status: 400, body: { error: `responsibleRole must be one of: ${ALL_ROLES.join(", ")}` } };
        return;
      }
      ...
      if (responsibleUserId) {
        ...
        if (!u) { result = { status: 400, body: { error: "responsibleUserId must belong to the same organization" } }; return; }
      }
      ...
      if (!(isTerminal ?? false) && !responsibleRole && !responsibleUserId) {
        result = { status: 400, body: { error: "Non-terminal stages must have a responsibleRole or responsibleUserId" } };
        return;
      }
  ...
  try {
    ...
    await withTenant(async () => {
      ...
      const [stage] = await db.update(wfTemplateStagesTable)
        ...
        .returning();
      result = stage ? { status: 200, body: stage } : { status: 404, body: { error: "Stage not found" } };
    });
    res.status(result!.status).json(result!.body);
  } catch (err: any) {
    logger.error({
  ...
  await withTenant(async () => {
    ...
    if (existing) {
      const stages = await db.select().from(wfTemplateStagesTable)
        ...
        .orderBy(asc(wfTemplateStagesTable.stageOrder));
      result = { status: 200, body: { message: "Invoice template already exists", template: { ...existing, stages } } };
      return;
    }
    ...
    result = { status: 201, body: { template: { ...tpl, stages: stages.sort((a, b) => a.stageOrder - b.stageOrder) } } };
  });
  res.status(result!.status).json(result!.body);
});
...
router.post("/instances", async (req, res): Promise<void> => {
  ...
  let result: { status: number; body: unknown } | undefined;
  let bundle: StageEmailBundle | null = null;
  await withTenant(async () => {
    // Verify document belongs to org
    const [doc] = await db.select().from(documentsTable)
      .where(and(eq(documentsTable.id, documentId), eq(documentsTable.organizationId, org))).limit(1);
    if (!doc) { result = { status: 404, body: { error: "Document not found or not in your organization" } }; return; }

    // Verify template belongs to org
    const [tpl] = await db.select().from(wfTemplatesTable)
      .where(and(eq(wfTemplatesTable.id, templateId), eq(wfTemplatesTable.organizationId, org))).limit(1);
    if (!tpl) { result = { status: 404, body: { error: "Template not found or not in your organization" } }; return; }
    ...
    const [dup] = await db.select().from(wfInstancesTable)
      ...
      .limit(1);
    if (dup) { result = { status: 409, body: { error: "An active workflow instance already exists for this document and template" } }; return; }
    ...
    // Sync document status → under_review (inside the tx now)
    await syncDocumentStatus(documentId, "under_review");
    ...
    // Notify stage responsible — DB side in-tx; email dispatched after commit.
    if (firstStage) bundle = await prepareStageNotification(inst, firstStage, req.user!.id);

    result = { status: 201, body: await enrichInstance(inst, req.user!, new Map()) };
  });
  if (result!.status !== 201) { res.status(result!.status).json(result!.body); return; }
  await dispatchStageEmail(bundle);
...
router.post("/instances/:id/advance", async (req, res): Promise<void> => {
  ...
  await withTenant(async () => {
    ...
    if (!inst) { result = { status: 404, body: { error: "Not found" } }; return; }
    if (inst.status !== "active") { result = { status: 409, body: { error: "Workflow is not active" } }; return; }
    ...
    if (!basis) {
      result = { status: 403, body: { error: "Forbidden", message: "You are not authorized to advance this workflow at its current stage" } };
      return;
    }
    ...
    if (newStatus === "completed") {
      ...
      await syncDocumentStatus(inst.documentId, docStatus);
      await createAuditLog({
        userId: req.user!.id,
        action: "status_change",
        entityType: "document",
    ...
    // Task lifecycle + notification for next stage (DB in-tx; email after commit)
    if (nextStage && newStatus === "active") {
      bundle = await prepareStageNotification(updated, nextStage, req.user!.id, "completed");
    } else if (newStatus === "completed") {
      // Workflow finished — close the final stage's task
      await closeOpenWorkflowTask(id, "completed");
    }

    result = { status: 200, body: await enrichInstance(updated, req.user!, new Map()) };
  });
...
router.post("/instances/:id/reject", async (req, res): Promise<void> => {
  ...
  await withTenant(async () => {
    ...
    if (!basis) {
      result = { status: 403, body: { error: "Forbidden", message: "You are not authorized to reject/return this workflow at its current stage" } };
      return;
    }
    if (basis === "admin_override") {
      await createAuditLog({
    ...
    if (finalAction === "rejected" || finalAction === "cancelled") {
      await syncDocumentStatus(inst.documentId, "draft");
      await createAuditLog({
        userId: req.user!.id,
        action: "status_change",
        entityType: "document",
    ...
    // (DB in-tx, email after commit); on reject/cancel, close current task as cancelled.
    if (finalAction === "returned" && returnStage) {
      bundle = await prepareStageNotification(updated, returnStage, req.user!.id, "cancelled");
    } else {
      await closeOpenWorkflowTask(id, "cancelled");
    }
...
router.post("/templates/:id/duplicate", requireRole("admin", "project_manager", "system_owner"), async (req, res): Promise<void> => {
  ...
  await withTenant(async () => {
    ...
    if (!orig) { result = { status: 404, body: { error: "Template not found" } }; return; }
    ...
    result = { status: 201, body: await getTemplateWithStages(copy.id, org) };
  });
  res.status(result!.status).json(result!.body);
});
...
router.post("/seed-defaults", requireRole("admin", "project_manager", "system_owner"), async (req, res): Promise<void> => {
  ...
  try {
    ...
    await withTenant(async () => {
      ...
      body = { results, templates: enriched };
    });
