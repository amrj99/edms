...
export class HuggingFaceProvider implements AIProviderClient {
  ...
  async chat(messages: ChatMessage[], options: ChatOptions = {}): Promise<ChatResult> {
    ...
    const prompt = allMessages.map(m => `${m.role === "user" ? "User" : m.role === "system" ? "System" : "Assistant"}: ${m.content}`).join("\n") + "\nAssistant:";
    ...
    const resp = await fetch(`https://api-inference.huggingface.co/models/${model}`, {
      ...
      body: JSON.stringify({
        inputs: prompt,
        parameters: { max_new_tokens: options.maxTokens ?? 512, temperature: options.temperature ?? 0.3, return_full_text: false },
      }),
    });
