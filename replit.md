...
- **Stripe Billing:** `artifacts/api-server/src/routes/billing.ts`. Plans: Starter 45 / Basic 65 / Professional 80 / Enterprise 95 AED/user/month. Endpoints: `GET /api/billing/plans`, `GET /api/billing/status`, `POST /api/billing/checkout` (Stripe Checkout Sessions), `POST /api/billing/portal` (Stripe Billing Portal), `POST /api/billing/webhook` (handles checkout.session.completed, subscription updated/deleted, invoice.payment_failed). Stripe data (customerId, subscriptionId) stored in `system_settings` as JSON keyed `stripe_org_{orgId}`. Frontend billing page at `/billing` with plan cards, seat selector, monthly total, current subscription card, Manage in Stripe button. Billing link added to admin sidebar.
  - **⚠️ Stripe NOT yet connected:** The Replit Stripe integration was dismissed. To activate payments, add these secrets manually: `STRIPE_SECRET_KEY` (from stripe.com dashboard → Developers → API keys), `STRIPE_WEBHOOK_SECRET` (from Stripe webhook endpoint config), and one price ID env var per plan: `STRIPE_PRICE_STARTER`, `STRIPE_PRICE_BASIC`, `STRIPE_PRICE_PROFESSIONAL`, `STRIPE_PRICE_ENTERPRISE`. Without these, the billing page shows plans but checkout/portal buttons return a 503 "Stripe not configured" error. The billing route gracefully handles the missing key case.
