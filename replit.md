...
- **FileDropZone Component:** Shared `artifacts/edms/src/components/file-drop-zone.tsx` with real drag-and-drop, click-to-browse, presigned URL upload (via `/api/storage/uploads/request-url`), multi-file support (via `multiple` + `onMultiUpload` props), progress feedback, and per-file removal.
...
- **Reports Export:** Reports page now has date range (From/To) filters, user filter, and Excel + PDF export buttons on each report tab (Document Register, RFI Log, Submittals, Transmittals). Excel uses `xlsx` library; PDF uses `jspdf` + `jspdf-autotable` with landscape layout and branded header.
