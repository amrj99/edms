...
- **Activity Log:** Full audit-trail page at `/activity-log` with DB-level pagination (50/page), role-gated (admin/project_manager/document_controller/system_owner), org-scoped CSV/XLSX exports, bilingual action/entity type labels (`ACTION_I18N`, `ENTITY_I18N`), and org-scoped user dropdown filter. API at `GET /api/audit-logs` with typed `SQL<unknown>[]` conditions. 44+ bilingual i18n keys added.
- **Module Licensing (Feature Flags per Org):** `modules` JSONB column added to `org_config` table (default: all 5 modules enabled). API at `GET/PUT /api/modules` (requireAuth + requireRole("admin","system_owner"); system_owner can target any org via `?orgId=N`). Frontend `useModules()` hook at `artifacts/edms/src/hooks/use-modules.ts`. Navigation in AppSidebar and notification bell in AppLayout dynamically hide based on module state. Admin panel gains a "Modules" tab (`ModulesTab` component) with toggle switches per module and org selector for system_owner role. 15 new bilingual i18n keys (EN+AR).
