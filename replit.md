...
- **System Admin Panel:** New admin tabs — Audit Log (filterable table + CSV export), System (live server stats, SMTP test, backup/restore), Email (SMTP status and trigger list). Backend at `GET /api/admin/system-info`, `POST /api/admin/smtp/test`, `GET /api/admin/backup`, `POST /api/admin/restore/validate`.
